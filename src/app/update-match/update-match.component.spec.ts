...
@Component({
  ...
  imports: [FormsModule, CommonModule, ReactiveFormsModule,
    MatCardModule, MatListModule, MatButtonModule, MatFormFieldModule, MatInputModule],
...
export class UpdateMatchComponent implements OnInit {
  ...
  openEditDialog(player: any): void {
    const dialogRef = this.dialog.open(PlayerEditDialogComponent, {
      width: '400px',
      data: { ...player } // Pass a copy of the player object
    });

    dialogRef.afterClosed().subscribe(result => {
      if (result) {
        // Update the player's data in the array
        const team = (player.team_obj.id === this.team1id) ? this.team1players : this.team2players;
